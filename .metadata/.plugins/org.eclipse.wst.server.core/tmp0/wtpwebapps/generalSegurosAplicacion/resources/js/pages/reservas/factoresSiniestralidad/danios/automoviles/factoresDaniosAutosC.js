var automovilesC = (function () {
	...
	$.ajax({
			...
			success: function(dataSet){
				...
				if(dataSet.mensaje === 'OK'){
					...
					for( item of dataSet.dataExtra ){
						if(item.ramo != ""){
							var aux = Number(item.pma_dev).toLocaleString('en-US',{maximumFractionDigits: "2", minimumFractionDigits: "2"});
							$('#P'+item.ramo+item.origen).html("$"+aux);
						}
					}
					var i;
					for (i = 0; i < 10; i++) {
						$('#'+item.origen).html(anio);
						anio--
					}
					var x;
					for(x=1; x <= 10; x++) {
			...
			statusCode: {
				404: function () {
					mensajes.modalAlert('warning','Error',"Verifique su conexión a internet codigo [404].");
					util.loadingEnd();
					$('#datatable-tablesPrima').addClass('d-none');
				}
			},
			error: function (xhr, status, error) {
				...
				console.log(xhr);
				mensajes.modalAlert('warning','Error',"La aplicación no esta disponible por el momento, codigo[500].");
				util.loadingEnd();
				$('#datatable-tablesPrima').addClass('d-none');
			},
			complete: function(){
	...
	$.ajax({
			...
			success: function(dataSet){
				...
				$("#datatable-tablesPrima").tableExport({
					formats: ["xlsx"], //Tipo de archivos a exportar ("xlsx","txt", "csv", "xls")
					position: 'button',  // Posicion que se muestran los botones puedes ser: (top, bottom)
					bootstrap: true,//Usar lo estilos de css de bootstrap para los botones (true, false)
					fileName: "PrimasDevengadas",    //Nombre del archivo 
				});
				
				$("#datatable-tablesTrianIndividual").tableExport({
					formats: ["xlsx"], //Tipo de archivos a exportar ("xlsx","txt", "csv", "xls")
					position: 'button',  // Posicion que se muestran los botones puedes ser: (top, bottom)
					bootstrap: true,//Usar lo estilos de css de bootstrap para los botones (true, false)
					fileName: "TriángulosAutosResidentes",    //Nombre del archivo 
				});
								
				$("#datatable-tablesTrianTuris").tableExport({
					formats: ["xlsx"], //Tipo de archivos a exportar ("xlsx","txt", "csv", "xls")
					position: 'button',  // Posicion que se muestran los botones puedes ser: (top, bottom)
					bootstrap: true,//Usar lo estilos de css de bootstrap para los botones (true, false)
					fileName: "TriánguloAutosTuristas",    //Nombre del archivo 
				});
				
					$("#datatable-tablesCamion").tableExport({
					formats: ["xlsx"], //Tipo de archivos a exportar ("xlsx","txt", "csv", "xls")
					position: 'button',  // Posicion que se muestran los botones puedes ser: (top, bottom)
					bootstrap: true,//Usar lo estilos de css de bootstrap para los botones (true, false)
					fileName: "TriángulosCamionesResidentes",    //Nombre del archivo 
				});  
