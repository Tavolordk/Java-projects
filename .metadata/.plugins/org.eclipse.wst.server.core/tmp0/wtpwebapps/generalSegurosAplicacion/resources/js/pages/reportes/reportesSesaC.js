var reportesSesaC = (function(){
	

	/*** METODO GENERA REPORTE CORS Y ORS ***/
	var reporteTrimestral = function(){
		...
		if(tipoReporte === ''){
			return mensajes.modalAlert('success', 'Información', 'Es Necesario elegir Tipo de Reporte');
		}
		...
		$.ajax({
			url : "ReportesSesaC/reporteCorsOrs/"+anioReporte+"/"+trimestre+"/"+tipoReporte,
			type: "GET",
			contentType : false,
			beforeSend  : function(){
				util.loadingStart();
	...
	return{
		reporteTrimestral : reporteTrimestral,
