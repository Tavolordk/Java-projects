var reporteRR4Trim = (function(){
	...
	var generaReporteRR4Trim = function(){
		 ...
		 if(anioReporte === null || anioReporte === '0'){
			...
			if(trimestre === ''){
				return mensajes.modalAlert('success', 'Información', 'Es Necesario elegir el Trimestre del Reporte');
			}
			
			$.ajax({
				url : "ReportesSesaC/reporteTrimRR4/"+anioReporte+"/"+trimestre,
				type: "GET",
				contentType : false,
				beforeSend  : function(){
					util.loadingStart();
				...
				if(dataSet.dataExtra !== null){
					var url  = URL.createObjectURL(utils(dataSet.dataExtra.bytes, "text/txt"));
					var blob = utils(dataSet.dataExtra.bytes, "text/txt");
					
					if (window.navigator.msSaveBlob) { // // IE hack; see http://msdn.microsoft.com/en-us/library/ie/hh779016.aspx
					    window.navigator.msSaveOrOpenBlob(blob, dataSet.dataExtra.nombre);
					}else {
					    var a  = window.document.createElement("a");
					    a.href = window.URL.createObjectURL(blob, { type: "text/txt" });
	...
	return{
		...
		generaReporteRR4Trim : generaReporteRR4Trim
