...
;(function ($, document, window, undefined) {
	...
	$.fn.userTimeout = function (opts) {
		var $defaults = {
			...
			ui: 'bootstrap',                        		// Model Dialog selector (auto, bootstrap, jqueryui)
		...
		var init = function() {
			...
			$(document).on('click keyup keypress', function () {
				startTimer();
				$.ajax({
					url: util.getPath() + "/sesionAlive",
					method: "GET",
					dataType: 'json',
		...
		var modal = function () {
			...
			if ($modalUI === 'bootstrap') {
				...
				$header = $('<div class="modal-header alert alert-warning"><h4 class="modal-title" id="notifyLogoutLabel">' + $options.modalTitle + '</h4><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button></div>');
				...
				$logoutBtn = $('<button type="button" class="btn btn-warning" id="logoff">' + $options.modalLogOffBtn + '</button>');
